...
const Cart = () => {
  ...
  console.log(productCartItems.map((items) => {
    return items
  }))

  console.log(TotalCartMap)
  ...
  const makingPayment = async (token) => {
    ...
    return await fetch(`${process.env.REACT_APP_SERVER}/payment`, {
      method : "POST",
      headers,
    ...
    .then((result) => {
      if (result.status === 200) {
        ...
        // Dispatch the clearCart action and navigate
        dispatch(clearCart());
        setTimeout(() => {
          navigate('/home');
        }, 1000);
